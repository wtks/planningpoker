...
3. **Real-time Communication**: WebSocket-based with defined message types in `server/src/types.ts`:
   - Client → Server: join, selectCard, revealCards, nextRound, leave, ping
...
- The client features automatic reconnection logic and a ping/pong heartbeat to maintain a stable connection.
